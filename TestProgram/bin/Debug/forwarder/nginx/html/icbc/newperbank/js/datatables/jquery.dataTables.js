...
(function($) {
	...
	_oExt.oPagination = {
	     ...
	     *           node:nPaging - the DIV which contains this pagination control
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				/*when mouseover display underline*/
				$(nFirst).mouseover(function(){
		            $(nFirst).css("text-decoration","underline");
		            $(nFirst).css("color","#CC0033");
	            });
		        $(nFirst).mouseout(function(){
		            $(nFirst).css("text-decoration","none");
		            $(nFirst).css("color","#000000");
		        });
		        $(nPrevious).mouseover(function(){
		            $(nPrevious).css("text-decoration","underline");
		            $(nPrevious).css("color","#CC0033");
	            });
		        $(nPrevious).mouseout(function(){
		            $(nPrevious).css("text-decoration","none");
		            $(nPrevious).css("color","#000000");
		        });
		        $(nNext).mouseover(function(){
		            $(nNext).css("text-decoration","underline");
		            $(nNext).css("color","#CC0033");
	            });
		        $(nNext).mouseout(function(){
		            $(nNext).css("text-decoration","none");
		            $(nNext).css("color","#000000");
		        });
		        $(nLast).mouseover(function(){
		            $(nLast).css("text-decoration","underline");
		            $(nLast).css("color","#CC0033");
	            });
		        $(nLast).mouseout(function(){
		            $(nLast).css("text-decoration","none");
		            $(nLast).css("color","#000000");
		        });
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
			{
				...
				if ( oSettings.oFeatures.bPageShow ){
	...
	_oExt.oSort = {
		...
		"numeric-comma-asc": function ( a, b )
		{
			...
			//	if()
			//}
			while( isNaN(a) && a.indexOf(",")!=-1 ){
				a = a.replace(",","");
			}
			while( isNaN(b) && b.indexOf(",")!=-1 ){
				b = b.replace(",","");
			}
			if(isNaN(a)&&isNaN(b))
				return 0;
		...
		"numeric-percent-asc": function ( a, b )
		{
			var reg = /,/g;
			var reg1 = /\r/g;
			var reg2 = /\n/g;
			var reg3 = / /g;
			var index = 0;
			index = a.toUpperCase().indexOf("</SCRIPT>");
			if(index>-1){
				a = a.slice(index+9,a.length-1);
			}
			a =  a.replace(reg, "").replace(reg1, "").replace(reg2, "").replace(reg3,"");	
			index = b.toUpperCase().indexOf("</SCRIPT>");
			if(index>-1){
				b = b.slice(index+9,b.length-1);
			}
			b =  b.replace(reg, "").replace(reg1, "").replace(reg2, "").replace(reg3,"");			
		...
		"numeric-percent-desc": function ( a, b )
		{
			var reg = /,/g;
			var reg1 = /\r/g;
			var reg2 = /\n/g;
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function classSettings ()
		{
			 ...
			 * Scope:    jQuery.dataTable.classSettings 
			 */
			this.oFeatures = { //设为默认样式，modify by tiangs
				"bPaginate": true,//分页
				"bLengthChange": false,//客户选择多少条分页
				"bFilter": false,//搜索栏
				"bSort": true,//排序
				"bInfo": true,//显示共几条第几条
			 ...
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.sPaginationType = "full_numbers";//two_button
			 ...
			 */
			this._iDisplayLength = 20;//原来是10
			this._iDisplayStart = 0;
			this._iDisplayEnd = 20;//原来是10
		 ...
		 */
		this.fnFilter = function( sInput, iColumn, bEscapeRegex, isEqual)
		{
			var oSettings = _fnSettingsFromNode( this[_oExt.iApiIndex] );
			...
			{
				/* Global filter */
				_fnFilterComplete( oSettings, {"sSearch":sInput, "bEscapeRegex": bEscapeRegex}, 1,isEqual );
			}
			else
			{
				...
				_fnFilterComplete( oSettings, oSettings.oPreviousSearch, 1,isEqual );
			}
		};
		...
		this.setPageLength = function(pagelength){
			var oSettings = this.fnSettings(this);
	        /* Redraw the table */
	        oSettings._iDisplayLength = pagelength;
	        _fnCalculateEnd( oSettings );
		 ...
		 * @return
		 */
		this.getColumnData = function (iColumn, bUnique, bFiltered, bIgnoreEmpty){
			var oSettings = this.fnSettings(this);
			var asResultData = new Array();
			if ( typeof iColumn == "undefined" ) return new Array();
			if ( typeof bUnique == "undefined" ) bUnique = true;
			if ( typeof bFiltered == "undefined" ) bFiltered = true;//默认显示数据
			...
			for (var i=0,c=aiRows.length; i<c; i++) {
				iRow = aiRows[i];
				aData = this.fnGetData(iRow);
				sValue = aData[iColumn];
				index = sValue.toUpperCase().indexOf("</SCRIPT>");
				if(index>-1){
					sValue =sValue.slice(index+9,sValue.length-1);
				}
				sValue= sValue.replace(reg,"").replace(reg1,"").replace(reg2,"").replace(reg3,"").replace(reg4,"");
		 ...
		 * @return
		 */
		this.getColumnsData = function (iColumn1,iColumn2, bUnique, bFiltered, bIgnoreEmpty){
			var oSettings = this.fnSettings(this);
			var asResultData = new Array();
			if ( typeof iColumn1 == "undefined" ) return new Array();
			if ( typeof iColumn2 == "undefined" ) return new Array();
			if ( typeof bUnique == "undefined" ) bUnique = true;
			if ( typeof bFiltered == "undefined" ) bFiltered = true;//默认显示数据
			if ( typeof bIgnoreEmpty == "undefined" ) bIgnoreEmpty = true;
			var aiRows;
			...
			for (var i=0,c=aiRows.length; i<c; i++) {
				...
				sValue1 = aData[iColumn2];
				index1 = sValue1.toUpperCase().indexOf("</SCRIPT>");
				if(index1>-1){
					sValue1 =sValue1.slice(index1+9,sValue1.length-1);
				}
				sValue1= sValue1.replace(reg,"").replace(reg1,"").replace(reg2,"").replace(reg3,"").replace(reg4,"");
		...
		/**
		 * 按第一列的值，统计第二列值的总数，第二列的值必须为数字类型
		 * @param col1
		 * @param col2
		 * @return
		 */
		this.sumCol2ByCol1 = function (col1,col2,bFiltered, bIgnoreEmpty){
			var col1Value = this.getColumnData(col1,true,bFiltered,bIgnoreEmpty);
			var oSettings = this.fnSettings(this);
			var asResultData = new Array();
			if ( typeof col1 == "undefined" ) return new Array();
			if ( typeof col2 == "undefined" ) return new Array();
			if ( typeof bFiltered == "undefined" ) bFiltered = true;//默认显示的数据
			if ( typeof bIgnoreEmpty == "undefined" ) bIgnoreEmpty = true;
			var aiRows;
			if (bFiltered == true) 
				aiRows = oSettings.aiDisplay; 
			...
			var reg4 = /\\/g;
			var arrValue;
			for(var k=0; k <col1Value.length;k++){//按第一列统计
				total =0;
				col1Data = col1Value[k];
				for (var i=0,c=aiRows.length; i<c; i++) {
					iRow = aiRows[i];
					aData = this.fnGetData(iRow);
					sValue1 = aData[col1];//第一列值
					sValue2 = aData[col2];//第二列值
					index = sValue1.toUpperCase().indexOf("</SCRIPT>");
					if(index>-1){
						sValue1 =sValue1.slice(index+9,sValue1.length-1);
					}
					sValue1 =  sValue1.replace(reg, "").replace(reg1, "").replace(reg2, "").replace(reg3,"").replace(reg4,"");	
					
					index = sValue2.toUpperCase().indexOf("</SCRIPT>");
					if(index>-1){
						sValue2 =sValue2.slice(index+9,sValue2.length-1);
					}
					sValue2 =  sValue2.replace(reg, "").replace(reg1, "").replace(reg2, "").replace(reg3,"").replace(reg4,"");
					
					if (bIgnoreEmpty == true && sValue2.length == 0) continue;
					
					var iValue2 = this.RMBYuanToFen(sValue2);
					if(col1Data==sValue1){
						total += parseInt(iValue2,10);	
					}
				}
				arrValue = col1Data + "=" + this.RMBFenToYuan(total);
				asResultData.push(arrValue);
			}
			return asResultData;
		}
		 ...
		 * @return
		 */
		this.sumCol4Stock = function (col1,col2,col3,bFiltered, bIgnoreEmpty){
			var col1Value = this.getColumnData(col1,true,bFiltered,bIgnoreEmpty);
			var col2Value = this.getColumnData(col2,true,bFiltered,bIgnoreEmpty);
			var col3Value = this.getColumnData(col3,true,bFiltered,bIgnoreEmpty);
			var oSettings = this.fnSettings(this);
			var asResultData = new Array();
			if ( typeof col1 == "undefined" ) return new Array();
			if ( typeof col2 == "undefined" ) return new Array();
			if ( typeof col3 == "undefined" ) return new Array();
			if ( typeof bFiltered == "undefined" ) bFiltered = true;//默认显示的数据
			if ( typeof bIgnoreEmpty == "undefined" ) bIgnoreEmpty = true;
			var aiRows;
			if (bFiltered == true) 
			...
			var arrValue ="hidden|";
				...
				for (var i=0,c=aiRows.length; i<c; i++) {
					...
					sValue1 = aData[col1];//第1列值
					sValue2 = aData[col2];//第2列值
					sValue3 = aData[col3];//第3列值
					...
					index = sValue3.toUpperCase().indexOf("</SCRIPT>");
					if(index>-1){
						sValue3 =sValue3.slice(index+9,sValue3.length-1);
					}
					sValue3 =  sValue3.replace(reg, "").replace(reg1, "").replace(reg2, "").replace(reg3,"").replace(reg4,"");
					
					if (bIgnoreEmpty == true && sValue1.length == 0 && sValue2.length == 0 && sValue3.length == 0) continue;
					
					var iValue1 = this.RMBYuanToFen(sValue1);
					var iValue2 = this.RMBYuanToFen(sValue2);
					var iValue3 = this.RMBYuanToFen(sValue3);
					
					total1 += parseInt(iValue1,10);	
					total2 += parseInt(iValue2,10);	
					total3 += parseInt(iValue3,10);	
					
				}
				arrValue = arrValue+ this.RMBFenToYuan(total1)+"|"+ this.RMBFenToYuan(total2)+"|"+ this.RMBFenToYuan(total3);
				asResultData.push(arrValue);
		...
		/**
		 * 根据查询回的结果，为formID.listID下拉框赋初值
		 * @param formID
		 * @param listID
		 * @param colsID
		 * @return
		 */
		this.setSelectList = function(listID,colsID1,colsID2){
			var obj= eval("document.all." + listID );
			var selectOptions = new Array();
			var listValue;
			var selLen;
			var keyValue;
			if ( typeof colsID2 == "undefined" ) {//如果无第二列，则取一列
				listValue = this.getColumnData(colsID1,true,false,true);
				selLen = listValue.length;
			selectOptions[0] = new Option("全部","");
			obj.options.add(selectOptions[0]);
			for(var k=1;k<=listValue.length;k++){
				selectOptions[k] = new Option(listValue[k-1],listValue[k-1]);
				obj.options.add(selectOptions[k]);
			}
			}else{
				listValue = this.getColumnsData(colsID1,colsID2,true,false,true);
				selLen = listValue.length;
				selectOptions[0] = new Option("全部","");
				obj.options.add(selectOptions[0]);
				
				for(var k=1;k<=listValue.length;k++){
					keyValue = listValue[k-1].split(" ")[0];
					selectOptions[k] = new Option(listValue[k-1],keyValue);
					obj.options.add(selectOptions[k]);
				}
			}
		...
		this.setSelectList4Stock = function(listID,colsID1,colsID2){
			var obj= eval("document.all." + listID );
			var selectOptions = new Array();
			var listValue;
			var selLen;
			...
			if ( typeof colsID2 == "undefined" ) {//如果无第二列，则取一列
				listValue = this.getColumnData(colsID1,true,false,true);
				selLen = listValue.length;
			selectOptions[0] = new Option("所有家庭成员","");
			obj.options.add(selectOptions[0]);
			for(var k=1;k<=listValue.length;k++){
				selectOptions[k] = new Option(listValue[k-1],listValue[k-1]);
				obj.options.add(selectOptions[k]);
		...
		/**
		 * 根据第colsID列对数据进行过滤,过滤后同时更新表数据
		 * @param keyValue
		 * @param colsID
		 * @return
		 */
		this.searchBy = function(listID,colsID,bEscapeRegex,statID, currCol, moneyCol, bFiltered, bIgnoreEmpty,sumCol,sumText,isEqual) {
			var oSettings = this.fnSettings(this);
			var oTable = this;
			if ( typeof bEscapeRegex == "undefined" ) {
				bEscapeRegex = true;
			}
			if ( typeof isEqual == "undefined" ) { //默认按非like匹配
				isEqual = true;
			}
			var obj= eval("document.all." + listID );
			$(obj).change(
				function() {
					oTable.fnFilter( obj.value, colsID ,bEscapeRegex,isEqual);
					//重新统计并显示
					if(typeof statID != "undefined"){
						oTable.statisticBy(statID, currCol, moneyCol, bFiltered, bIgnoreEmpty,sumCol,sumText);
					}
				}
			);
			$(obj).change();
		...
		/**
		 * 根据第colsID列对数据进行过滤,过滤后同时更新表数据，股票专用
		 * @param keyValue
		 * @param colsID
		 * @return
		 */
		this.searchBy4Stock = function(listID,colsID,bEscapeRegex,statID, moneyCol1,moneyCol2,moneyCol3, bFiltered, bIgnoreEmpty,sumCol,sumText,isEqual) {
			var oSettings = this.fnSettings(this);
			var oTable = this;
			if ( typeof bEscapeRegex == "undefined" ) {
				bEscapeRegex = true;
			...
			$(obj).change(
				function() {
					...
					//重新统计并显示
					if(typeof statID != "undefined"){
						oTable.statisticBy4Stock(statID, moneyCol1,moneyCol2,moneyCol3, bFiltered, bIgnoreEmpty,sumCol,sumText);
					}
				}
			);
			$(obj).change();
		 ...
		 * @return
		 */
		this.searchByTwo = function(listID,colsID,colsID2,bEscapeRegex,statID, currCol, moneyCol, bFiltered, bIgnoreEmpty,sumCol,sumText,isEqual) {
			var oSettings = this.fnSettings(this);
			var oTable = this;
			if ( typeof bEscapeRegex == "undefined" ) {
				bEscapeRegex = true;
			...
			$(obj).change(
				function() {
					...
					var selText = document.getElementById(listID).options[selIndex].text;
					if(typeof colsID2 == "undefined"  || obj.value==""){
						oTable.fnFilter(obj.value, colsID ,bEscapeRegex,isEqual);
						oTable.fnFilter("", colsID2 ,bEscapeRegex,isEqual);
					}else{
						var temp = selText.split(" ");
						oTable.fnFilter( obj.value, colsID ,bEscapeRegex,isEqual);
						oTable.fnFilter( temp[1], colsID2 ,bEscapeRegex,isEqual);		
					}
					//重新统计并显示
					if(typeof statID != "undefined"){
						oTable.statisticBy(statID, currCol, moneyCol, bFiltered, bIgnoreEmpty,sumCol,sumText);
		 ...
		 * @return
		 */
		this.searchByMore = function(listID,colsIDS,bEscapeRegex,statID, currCol, moneyCol, bFiltered, bIgnoreEmpty,sumCol,sumText,isEqual) {
			var oSettings = this.fnSettings(this);
			var oTable = this;
			if ( typeof bEscapeRegex == "undefined" ) {
				bEscapeRegex = true;
			...
			$(obj).change(
				function() {
					searchCols = colsIDS.split("|");
					var selIndex =  document.getElementById(listID).selectedIndex;
					var selText = document.getElementById(listID).options[selIndex].text;
					if(searchCols.length==0){//没有设置参数
						oTable.fnFilter("", 1 ,bEscapeRegex,isEqual);
					}else if(searchCols.length==1){//1个参数
						oTable.fnFilter(obj.value, searchCols[0] ,bEscapeRegex,isEqual);
					}else{//多个参数
						if(obj.value==""){//如果选择全部
							for(var j=0; j<searchCols.length;j++){
								oTable.fnFilter("", searchCols[j] ,bEscapeRegex,isEqual);
							}
						}else{
							var temp = selText.split(" ");
							for(var k=0; k<searchCols.length;k++){
								oTable.fnFilter(temp[k], searchCols[k] ,bEscapeRegex,isEqual);
							}
						}
					}
					//重新统计并显示
		 ...
		 * @return
		 */
		this.statisticBy = function(statID,currCol,moneyCol,bFiltered, bIgnoreEmpty,sumCol,sumText){
			var tList= document.getElementById(statID);
		    while(tList.rows.length>1){
		    	tList.deleteRow(-1);
		    }

			//获取统计结果
			var statList = this.sumCol2ByCol1(currCol,moneyCol,bFiltered,bIgnoreEmpty);
			var len = statList.length;
			var newTr;
			var newCell;
			var temp;
			var colsNum = tList.rows[0].cells.length;
			for(var i =0;i<len;i++){
				temp = statList[i].split("=");
				if(temp[0]=="hidden"){
					temp[0]="";
				}
				newTr = tList.insertRow(-1);
				...
				for(var k=0; k<colsNum;k++){
					if(k==sumCol){
						...
						newCell.innerHTML = "<div align=right>"+sumText+"</div>";
						}else{//显示空
							newCell = newTr.insertCell(-1);
							newCell.innerHTML = "&nbsp;";
						}
						...
						newCell = newTr.insertCell(-1);
						newCell.innerHTML ="<div align=center>"+ temp[0]+"</div>";
					}else if(k==moneyCol){
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "<div align=right><b>"+this.convertToMoney(temp[1])+"</b></div>";
			}else{
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "&nbsp;";
					}
		...
		this.statisticBy4Stock = function(statID,moneyCol1,moneyCol2,moneyCol3,bFiltered, bIgnoreEmpty,sumCol,sumText){
			var tList= document.getElementById(statID);
		    while(tList.rows.length>1){
		    	tList.deleteRow(-1);
		    }
			...
			//获取统计结果
			var statList = this.sumCol4Stock(moneyCol1,moneyCol2,moneyCol3,bFiltered,bIgnoreEmpty);
			...
			for(var i =0;i<len;i++){
				...
				newTr = tList.insertRow(-1);
				//newTr.style.borderCollapse="separate";
				//newTr.style.backgroundColor = "#E6E6E6";
				newTr.className = "bc3";
				...
				for(var k=0; k<colsNum;k++){
					if(k==sumCol){
						if(i==0){//第零行显示统计名称
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "<div align=center>"+sumText+"</div>";
						}else{//显示空
							newCell = newTr.insertCell(-1);
							newCell.innerHTML = "&nbsp;";
						}
						...
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "<div align=center>"+this.convertToMoney(temp[1])+"</div>";
					}else if(k==moneyCol2){
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "<div align=center>"+this.convertToMoney(temp[2])+"</div>";
					}else if(k==moneyCol3){
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "<div align=center>&nbsp;"+this.convertToMoney(temp[3])+"</div>";
					}
					else{
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "&nbsp;";
		 ...
		 * @return
		 */
		this.searchByDetail = function(listID,colsID,bEscapeRegex,statID, currCol, moneyCol1,moneyCol2, bFiltered, bIgnoreEmpty,sumCol,sumText,banlanceCol,balanceText,showMoney,isEqual) {
			var oSettings = this.fnSettings(this);
			var oTable = this;
			if ( typeof bEscapeRegex == "undefined" ) {
				bEscapeRegex = true;
			...
			$(obj).change(
				function() {
					...
					//重新统计并显示
					if(typeof statID != "undefined"){
						oTable.statisticByDetail(statID, currCol, moneyCol1,moneyCol2,bFiltered, bIgnoreEmpty,sumCol,sumText,banlanceCol,balanceText,showMoney);
					}
				}
			);
			$(obj).change();
		 ...
		 * @return
		 */
		this.statisticByDetail = function(statID,currCol,moneyCol1,moneyCol2,bFiltered, bIgnoreEmpty,sumCol,sumText,banlanceCol,balanceText,showMoney){
			var tList= document.getElementById(statID);
		    while(tList.rows.length>1){
		    	tList.deleteRow(-1);
		    }
		    if(typeof showMoney=="undefined"){//默认显示币种列
		    	showMoney = true;
		    }

			//获取统计结果
			var statList = this.sumCol2ByCol1(currCol,moneyCol1,bFiltered,bIgnoreEmpty);
			var statList1 = this.sumCol2ByCol1(currCol,moneyCol2,bFiltered,bIgnoreEmpty);
			...
			for(var i =0;i<len;i++){
				...
				for(var k=0; k<colsNum;k++){
					if(k==sumCol){
						...
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "<div align=right><b>"+this.convertToMoney(temp1[1])+"</b></div>";
					}else{
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "&nbsp;";
					}
					balanceTemp = this.RMBYuanToFen(temp[1]) - this.RMBYuanToFen(temp1[1]);
					balance[i] = this.RMBFenToYuan(balanceTemp);
				}
			}
			for(i =0;i<len;i++){
				temp = statList[i].split("=");
				...
				for(k=0; k<colsNum;k++){
					if(k==banlanceCol){
						if(i==0){//第零行显示统计名称
							newCell = newTr.insertCell(-1);
							newCell.innerHTML = "<div align=right>"+balanceText+"</div>";
						}else{//显示空
							newCell = newTr.insertCell(-1);
							newCell.innerHTML = "&nbsp;";
						}
						...
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "<div align=right><b>"+this.convertToMoney(balance[i])+"</b></div>";
					}else{
						newCell = newTr.insertCell(-1);
						newCell.innerHTML = "&nbsp;";
					}
		 ...
		 * @return
		 */
		this.convertToMoney = function (number){
			var result;
		 ...
		 * @return
		 */
		this.RMBYuanToFen = function(number){
			var result = "";
			var smallPart;
			var bigPart;
			var smallLen;
			number = parseFloat(number,10);
			var number1 = ''+number; //转化为字符串
			var pIndex = number1.indexOf("."); //判断是否有点
			
			if(pIndex>-1){
				bigPart = number1.substring(0,pIndex);
				smallPart = number1.substring(pIndex,number1.length);
			}else{
				bigPart = number1;
			}
			//小数部分补齐两位
			...
			//小数部分保留两位
			smallPart = smallPart.substring(1,3);
			result = ''+bigPart + smallPart;
		 ...
		 * @return
		 */
		this.RMBFenToYuan = function(number){
			number = parseInt(number,10);
			var result = '' + number;
			var fushu = false;
			if(result.charAt(0)=="-"){//如果是负数
			...
			if(len>2){
				bigPart = result.substring(0, len-2);
				smallPart = result.substring(len-2, len);
			}else{
				bigPart="0";
				smallPart = result;
				smallLen = result.length;
				for(var i=0;i<2-smallLen;i++){
					smallPart = "0" + result;
				}
			}
			if(fushu==false)
		 ...
		 */
		function _fnFilterComplete ( oSettings, oInput, iForce, isEqual )
		{
			if(typeof isEqual=="undefined")
			{
				/* 默认模糊匹配 */
			...
			for ( var i=0 ; i<oSettings.aoPreSearchCols.length ; i++ )
			{
				_fnFilterColumn( oSettings, oSettings.aoPreSearchCols[i].sSearch, i, 
					oSettings.aoPreSearchCols[i].bEscapeRegex,isEqual  );
			}
		 ...
		 */
		function _fnFilterColumn ( oSettings, sInput, iColumn, bEscapeRegex,isEqual )
		{
			if ( sInput === "" )
			{
				return;
			...
			for ( var i=oSettings.aiDisplay.length-1 ; i>=0 ; i-- )
			{
				...
				if(isEqual==true){
					...
					sData =  sValue1.replace(reg, "").replace(reg1, "").replace(reg2, "").replace(reg3, "").replace(reg4, "");
					sRegexMatch = sRegexMatch.toString().replace(reg4, "");
					if(sRegexMatch!= sData){
						oSettings.aiDisplay.splice( i, 1 );
						iIndexCorrector++;
					}
				}else{
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
				...
				(oSettings.aaSorting.length !== 0 || oSettings.aaSortingFixed !== null) )
			{
				...
				if ( !window.runtime )
				{
					...
					eval( sDynamicSort );
					//此处只显示过滤后的数据
					oSettings.aiDisplay.sort( fnLocalSorting );
			...
			{
				//此处只显示过滤后的数据
				oSettings.aiDisplay = oSettings.aiDisplay.slice();
				//oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
		...
		function _fnUpdateInfo ( oSettings )
		{
			...
			else if ( oSettings.fnRecordsDisplay() == oSettings.fnRecordsTotal() )
			{
				...
				nFirst.innerHTML = oSettings.oLanguage.sInfo.
						...
						replace('_TOTALPAGE_',Math.ceil((oSettings.fnRecordsDisplay()) / oSettings._iDisplayLength)).
						replace('_CURRENTPAGE_',Math.ceil(oSettings._iDisplayStart / oSettings._iDisplayLength) + 1).
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings.oFeatures, oInit, "bPageShow" );
				...
				}else{  //第一列序号在排序或过滤时重新编号 add by tiangs
					oSettings.aoDrawCallback.push( {
						"fn": function fnDrawCallback( oSettings ) {
