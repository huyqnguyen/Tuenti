...
namespace Levenshtein_distance
{
    public class Program
    {
        static void Main(string[] args)
        {
            var str1 = "astuinge";
            var str2 = "stringe";
            Console.WriteLine(GetDistance(str1, str2));
            //for (int i = 1; i < 100000000; i++)
            //{
            //    for (int f = 0; f < 100000000-1; f++)
            //    {
            //        if ((i - f) == 1 && (Math.Pow(i, 3) - Math.Pow(f, 3)) == 1)
            //        {
            //            Console.WriteLine("{0} - {1}", i, f);
            //            Console.WriteLine("{0}", Math.Pow(i, 4) - Math.Pow(f, 4));
            //        }
            //    }
            //}
            Console.Read();
        ...
        public static int GetDistance(string st1, string st2)
        {
            // degenerate cases
            if (st1 == st2) return 0;
